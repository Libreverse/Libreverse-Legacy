...
- Rails 8 full-stack app (Libreverse-Legacy); Ruby 3.3.7 via rbenv (`.ruby-version`); JS via Bun (`bun.lock`), gems via Bundler; production Docker uses Phusion Passenger (Rodauth, mounted engines, Turbo/ActionCable).
...
- CI TiDB rails-test: TLS required (`TIDB_TEST_SSL_MODE=VERIFY_IDENTITY`); create DB via `RAILS_ENV=test DISABLE_EAGER_LOAD=true rails db:create` before `db:test:prepare`; bootstrap `google_robotstxt_parser` via `scripts/bootstrap-robotstxt-sources.sh`.
